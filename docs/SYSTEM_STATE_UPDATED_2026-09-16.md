...
Все 8 миграций применены в локальной БД.
...
Локально уже существует и применена:

```text
1788859125918_training_templates.js
```

Поэтому миграция `5918` пока относится к актуальному локальному состоянию.

Production migration для `5918` в рамках текущего локального этапа не
выполнялась.
...
В ходе проверки реальный S3 object не создавался и не удалялся.
...
- полноценный Media API;
- сохранение media metadata/link в PostgreSQL;
- permission-aware signed URL flow;
...
На 16.09.2026 фундамент локальной разработки приведён к рабочему состоянию:
